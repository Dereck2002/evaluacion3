...
switch ($_GET["op"]) {
    ...
    case 'insertar':
        ...
        $datos = array(); //defino un arreglo
        $datos = $proveedor->insertar($Nombre, $Producto_suministrado, $Fecha_inicio_contrato); //llamo al modelo de usuarios e invoco al procedimiento insertar
    ...
    case 'eliminar':
        $ID_proveedor = $_POST["ID_proveedor"]; //defino una variable para almacenar el id del usuario, la variable se obtiene mediante POST
        $datos = array(); //defino un arreglo
        $datos = $proveedor->eliminar($ID_proveedor); //llamo al modelo de usuarios e invoco al procedimiento uno y almaceno en una variable
        echo json_encode($datos); //devuelvo el arreglo en formato json
        break;
}
